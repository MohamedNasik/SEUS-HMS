...
$username= $row['p_fname'].' '. $row['p_lname'];
   ...
   $stmt = $conn->prepare("SELECT * FROM testing_schedule WHERE  test_id='2' AND pres_id=? ");
   $testname =  $testname;
      ...
      echo "ERROR: Could not save : $sql. " . mysqli_error($conn);
       ...
       if( $stmt2 = mysqli_prepare($conn,"UPDATE testing_schedule SET lab_status =? WHERE testing_schedule_id =?")){
        ...
        /* BK: always check whether the execute() succeeded */
             ...
             $to      = $email;
             $subject = 'Urine Test Status';
             $message = '<p> Dear <b> ' .$username.' </b> <br>  Your Urine Test has been successfully completed. Confirm your payment for the test to view the test report. <br><br>  Report ID: ' .$report_id.'  <br> Prescription ID: ' .$presid. '  <br> Submitted Date: ' .date("Y-m-d")  .'  <br><br>Thank You. <br><br> Best Regards from </br> <br><b> <i>  SEUS Hospitals Laboratory </i></b> </br>';
